...
PEER_REVIEW_APPROVERS=$(get_env peer_review_approvers "1")
...
if [[ $APP_SCM_TYPE == "gitlab" ]]; then
  ...
  curl -H "Authorization: Bearer $(cat ${APP_TOKEN_PATH})" "${APP_API_URL}/repos/${APP_REPO_OWNER}/${APP_REPO_NAME}/branches/master/protection" \
    -XPUT -d "{\"required_pull_request_reviews\":{\"dismiss_stale_reviews\":true, \"required_approving_review_count\":${PEER_REVIEW_APPROVERS}},\"required_status_checks\":{\"strict\":true,\"contexts\":[\"tekton/code-branch-protection\",\"tekton/code-unit-tests\",\"tekton/code-cis-check\",\"tekton/code-vulnerability-scan\",\"tekton/code-detect-secrets\"]},\"enforce_admins\":null,\"restrictions\":null}"
fi
npm ci
